...
/** Update Post Like Feature */
export const useLikePost = () => {
    const queryClient = useQueryClient();

    return useMutation({
        mutationFn: ({ postId, likesArray }: { postId: string | undefined; likesArray: string[] }) => likePost(postId || '', likesArray),
        onSuccess: (data) => {
            queryClient.invalidateQueries({
...
/** Get Search results */
export const useSearchPosts = (searchItem: string) => {
    return useQuery({
        queryKey: [QUERY_KEYS.SEARCH_POSTS, searchItem],
        queryFn: () => searchPosts(searchItem),
        enabled: !!searchItem
    })
}